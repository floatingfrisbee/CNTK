...
class ProgressPrinter(cntk_py.ProgressWriter):
    ...
    def ___write_progress_update(self, samples, updates, aggregate_loss, aggregate_metric, frequency, name):
        ...
        if frequency == 0:
            if aggregate_loss is not None:
                avg_loss_since_start = (aggregate_loss[1] / samples[1]) if samples[1] > 0 else 0.0
                loss_increment = (aggregate_loss[1] - aggregate_loss[0])
                avg_loss_since_last = (loss_increment / samples_since_last) if samples_since_last > 0 else 0.0
            ...
            if aggregate_metric is not None:
                avg_metric_since_start = (aggregate_metric[1] / samples[1]) if samples[1] > 0 else 0.0
                metric_increment = aggregate_metric[1] - aggregate_metric[0]
                avg_metric_since_last = (metric_increment / samples_since_last) if samples_since_last > 0 else 0.0
                format_str += '{:8.3g}   {:8.3g}   '
    ...
    def on_write_training_summary(self, samples, updates, summaries, aggregate_loss, aggregate_metric,
        ...
        speed = samples / elapsed_seconds if elapsed_seconds > 0 else 0.0
        avg_loss = (aggregate_loss / samples) if samples > 0 else 0.0

        if aggregate_metric is not None:
            avg_metric = (aggregate_loss / samples) if samples > 0 else 0.0
    ...
    def on_write_evaluation_summary(self, samples, updates, summaries, aggregate_metric, elapsed_milliseconds):
        ...
        avg_metric = (aggregate_metric / samples) if samples > 0 else 0.0
...
class TensorBoardProgressWriter(cntk_py.ProgressWriter):
    ...
    def on_write_training_update(self, samples, updates, aggregate_loss, aggregate_metric):
        # Override for ProgressWriter.on_write_training_update().
        total_samples = samples[1] - samples[0]

        loss_increment = aggregate_loss[1] - aggregate_loss[0]
        avg_loss = (loss_increment / total_samples) if total_samples > 0 else 0.0
        self.write_value('minibatch/avg_loss', avg_loss, updates[1])

        metric_increment = aggregate_metric[1] - aggregate_metric[0]
        avg_metric = (metric_increment) / total_samples if total_samples > 0 else 0.0
    ...
    def on_write_training_summary(self, samples, updates, summaries, aggregate_loss, aggregate_metric,
        ...
        avg_loss = (aggregate_loss / samples) if samples > 0 else 0.0
        self.write_value('summary/avg_loss', avg_loss, summaries)

        avg_metric = (aggregate_metric / samples) if samples > 0 else 0.0
        self.write_value('summary/avg_metric', avg_metric, summaries)
